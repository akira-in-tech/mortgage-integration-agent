...
Every route under `/v1/loan-cases`, `/v1/webhook-endpoints`, `/v1/webhook-deliveries`, and `/v1/loan-cases/:caseId/communication-messages` is guarded by `TenantAuthGuard` (`src/auth/`, M5-024) — a composite: a request authenticates with *either* a machine `Authorization: Bearer {clientId}.{secret}` credential (`ApiKeyGuard`, `ApiClient`, Section 20 M5's original "scoped API-client" target) *or* a real OIDC bearer token from a real issuer plus an `X-Tenant-Id` header (`OidcGuard`, `User`/`TenantMembership`, Section 14.1, Section 16.1's "OIDC/OAuth 2.0 for people"). NestJS's own guard composition is AND-only, so `TenantAuthGuard` implements the OR itself: try `ApiKeyGuard` (cheap, no network call, its own shape check rejects a JWT-shaped token instantly), fall back to `OidcGuard` only if that fails. Both resolve to the identical `AuthContext` shape (`tenantId`/`actorId`/`role`/`correlationId`) every downstream consumer (`RoleGuard`, `AuthTenantId()`, `CurrentAuth()`, every controller's own audit-event calls) reads uniformly, never knowing or caring which credential type authenticated the request. There is no self-service credential endpoint for either model (the same honest gap tenant creation itself already has): `npm run create-api-client -- <tenantId> <name> [PARTNER|REVIEWER]` mints a machine credential; `npm run manage-user -- create-user <subject> <email>` then `grant-membership <userId> <tenantId> <PARTNER|REVIEWER>` provisions a human one. Neither `api_clients` nor `tenant_memberships` has row-level security — both exist specifically to *resolve* which tenant a request may act in, so RLS could never apply to that lookup itself (see each entity's own comment). The tenant a request acts as is always resolved this way, never from a request body — `CreateCaseDto`/`CreateWebhookEndpointDto` have no `tenantId` field at all, so there is nothing left for a caller to get right or wrong. A case (or webhook endpoint, or delivery) owned by a different tenant 404s exactly like a nonexistent one would for a caller with sufficient role, never a 403 that would leak whether the resource exists. **Real OIDC, not a hand-rolled scheme**: `OidcService` uses real OpenID Connect Discovery + `jose`'s real remote-JWKS verification against a real, self-hosted Keycloak (`docker-compose.yml`'s `keycloak` service, seeded from `keycloak/realm-export.json` — a real realm, a public client with an audience mapper, one real test user). `jose@6` is pure ESM with no CommonJS build; this codebase compiles to CommonJS, so `jose@4` (the last version with a real CJS entry point) is what's actually installed — a real, if slightly dated, compatibility constraint, not a design choice. This codebase never implements its own OIDC *authorization server* — only a real *relying party* against one. See `docs/DEVELOPMENT_LOG.md`'s M5-001 entry for the original scope decision and a real NestJS testing-injector quirk that slice found, and the M5-024 entry for this one.
