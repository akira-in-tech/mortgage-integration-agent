...
**Policy-impact check (M7-018)** is the third of the four items in that same charter list. `checkPolicyChangeImpact` already had both a REST route and a GraphQL mutation (M6-004); this slice just gave it a console UI — a small control on a case's own Overview tab that answers "would this policy version change how this case resolves?" without changing anything. There's still no way to browse existing policy versions from the console or API, so a reviewer has to already know the id they want to check; the UI says so plainly rather than hiding the gap.
...
Known gaps, honestly scoped rather than silently cut: client-side-only search on the Triage Queue, encryption-key rotation currently invalidates existing OIDC sessions rather than supporting a multi-key decrypt window (now true for both `ApiClient` and `PlatformAdmin` bearer tokens), and no way to browse existing policy versions. The real Keycloak/API/PostgreSQL browser journey now runs by default in hosted CI (M7-022), alongside the fast fixture-based Playwright/axe gate.
...
`src/agent-runtime/langgraph/lending-operations-agent-runtime.ts` is `AgentRuntimePort`'s first implementation: a real, compiled LangGraph.js v1 `StateGraph` that orchestrates four live tools in Section 9.5's Agent-loop order (verify consent → check completeness → request a guarded policy evaluation → propose/execute a condition transition → draft a protected remediation request when needed), enforcing `allowedTools` by only registering the tools a run names. Trusted duration, step, token, provider-call, and monetary limits are enforced through PostgreSQL-authoritative ledgers and reservations at every tool boundary; current tools honestly declare zero token/provider/cost consumption because this graph still makes no model or outbound-provider calls itself. `case-conditions.activities.ts`'s `evaluateConditions` calls this runtime instead of the policy engine and condition tool directly. Condition and case-readiness writes are compare-and-swap protected against `LoanCase.version`; policy ambiguity interrupts durably for review rather than being overwritten or treated as a final decision. Most of Section 9.6's evidence-quality, contract-conformance, and model-safety trigger categories remain open until their backing signals exist.
