...
**Data-disposition review on consent revocation (M5-015) — Section 14.1's `data_disposition_tasks`, which didn't exist in this codebase at all before this slice.** `ConsentService.revoke()` now calls `DataDispositionService.openRetentionReviewForRevokedConsent()` inside its own transaction (not a separate call afterward — a revocation can never commit without also opening its required review), opening a real, RLS-protected `DataDispositionTask` row (`taskType=RETENTION_REVIEW`, `status=PENDING`) that snapshots every `evidence_facts` id the case had at that exact moment, plus the id of the consent record that triggered it. The task is opened, not acted on — matching the charter's own "opens a review" phrasing and Section 6.3's human-in-the-loop posture: no automated deletion, anonymization, or verification runs against it, and `DataDispositionTaskType`/`Status`'s fuller vocabulary (`DELETION`/`ANONYMIZATION`/`LEGAL_HOLD`, `IN_PROGRESS`/`COMPLETED`/`VERIFIED`) is declared but never driven yet — the same honest-undriven-state pattern `ProviderOperationIntentStatus`'s `RECONCILING`/`CANCELLED` already established. `listForCase()` exists and is tested but has no controller/resolver yet — Section 15.2's "data-disposition work" GraphQL surface is explicitly M6 UI-milestone scope. Section 14.2's fuller lineage requirement (document, normalized-finding, cache, search-index, backup) is far larger than this slice and unbuilt — this codebase has none of those subsystems to traverse, so only `evidence_facts` is snapshotted. Incidentally found and fixed while live-verifying this slice: the checked-in `openapi/openapi.json`/`client/generated/schema.d.ts` had been stale since M5-005 shipped the consents endpoint (the artifact's own description still claimed it "is not yet built") — regenerated both.
