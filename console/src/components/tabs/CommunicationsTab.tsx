...
export function CommunicationsTab({ loanCase }: { loanCase: LoanCase }) {
  ...
  const [approve, approveState] = useMutation(APPROVE_COMMUNICATION_MESSAGE_MUTATION, {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {messages.map((message) => (
        <div key={message.id} className="card" style={{ padding: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ fontSize: 13, fontWeight: 600 }}>{message.classification} &middot; {message.channel}</div>
            <div style={{ fontSize: 11.5, color: 'var(--ink-muted)', marginTop: 2 }}>
              {message.status} &middot; drafted {formatDateTime(message.createdAt)}
            ...
            {message.status === 'AWAITING_APPROVAL' && (
              <button
                ...
                onClick={() => {
                  ...
                  if (!actorId) return;
                  approve({ variables: { messageId: message.id, input: { actorId } } });
                }}
              >
                Approve
              </button>
