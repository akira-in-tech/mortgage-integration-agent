...
6. **Phase 1 done and live (M7-024)**: Terraform + GitHub OIDC deployment for API/Worker/a self-hosted Temporal server, live on real AWS infrastructure — real `terraform apply`, a real one-off migration task exiting 0 against a real RDS instance, real ECS services at steady state, and a real `200` from the deployed ALB (`curl` verified independently, not just the workflow's own check). Getting there for real surfaced 18 genuine bugs across IAM permissions, Terraform's dependency graph, and the application's own migration/SSL assumptions — all recorded in `docs/DEVELOPMENT_LOG.md`'s M7-024 entry, not glossed over. **In progress (M7-025)**: a `staging-drill.yml` workflow for load/soak testing, a real backup/restore drill (real RDS snapshot, real restore into a temporary instance, real query verifying the data), and a failure-recovery exercise (kill the running API task, measure the real ALB recovery window) — built and structurally validated, not yet run for real. **Still open**: Keycloak/console deployment (needs a real domain — HTTPS is a hard requirement whenever OIDC is configured in staging/production, discovered while scoping this work), and immutable-artifact supply-chain evidence (SBOM/provenance attestation — ECR image immutability itself is done).
