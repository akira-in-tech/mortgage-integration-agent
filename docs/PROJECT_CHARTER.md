...
Real progress so far: SLO dashboards, alerts, and a runbook (`docs/OPERATIONS.md`) are built, running on a free local OpenTelemetry stack. CI enforces console/backend tests, generated-contract drift, a container build, dependency audits, and (M7-021) real secret scanning and SAST; a browser and automated-accessibility gate runs by default, including (M7-022) the live-Keycloak/PostgreSQL login journey, no longer opt-in only. Terraform + GitHub OIDC deployment for a real (Phase 1) AWS staging environment is written and structurally validated (M7-024) but not yet live-deployed. Still fully open: deployed synthetic staging's own live verification, load/soak, backup/restore, and the provider certification/kill-switch exercise.
...
6. **In progress (Phase 1, M7-024)**: Terraform + GitHub OIDC deployment for API/Worker/a self-hosted Temporal server on real AWS infrastructure, with a real, disclosed cost estimate — structurally validated, not yet live-verified pending a real account's first `deploy-staging.yml` run. **Still open**: Keycloak/console deployment (Phase 2), immutable-artifact supply-chain evidence (SBOM/provenance attestation — ECR image immutability itself is done), backup/restore drill, load/soak testing, and a formal failure-recovery exercise — all of which need Phase 1's infrastructure to actually exist and be proven first.
