...
Real progress so far: SLO dashboards, alerts, and a runbook (`docs/OPERATIONS.md`) are built, running on a free local OpenTelemetry stack. CI enforces console/backend tests, generated-contract drift, a container build, dependency audits, and (M7-021) real secret scanning and SAST; a browser and automated-accessibility gate runs by default, including (M7-022) the live-Keycloak/PostgreSQL login journey, no longer opt-in only. Terraform + GitHub OIDC deployment for a real (Phase 1) AWS staging environment is live and verified against a real account (M7-024) — a real ALB `200`, not just a structural check. Still fully open: load/soak, backup/restore, a formal failure-recovery exercise, and the provider certification/kill-switch exercise.
...
6. **Phase 1 done and live (M7-024)**: Terraform + GitHub OIDC deployment for API/Worker/a self-hosted Temporal server, live on real AWS infrastructure — real `terraform apply`, a real one-off migration task exiting 0 against a real RDS instance, real ECS services at steady state, and a real `200` from the deployed ALB (`curl` verified independently, not just the workflow's own check). Getting there for real surfaced 18 genuine bugs across IAM permissions, Terraform's dependency graph, and the application's own migration/SSL assumptions — all recorded in `docs/DEVELOPMENT_LOG.md`'s M7-024 entry, not glossed over. **Still open**: Keycloak/console deployment (Phase 2), immutable-artifact supply-chain evidence (SBOM/provenance attestation — ECR image immutability itself is done), backup/restore drill, load/soak testing, and a formal failure-recovery exercise.
