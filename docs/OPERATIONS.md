...
`OIDC_SESSION_ENCRYPTION_KEYS` is the same `id:64hex[,id:64hex...]` ring shape, applied to durable OIDC session rows (`oidc_sessions.encryptedTokenBundle`) instead — but with no backfill script, deliberately: a session is short-lived (`OIDC_SESSION_MAX_AGE_SECONDS`, default 8h) and every real read path (`authenticateCookie()`, including a routine token refresh) re-encrypts under whichever key is currently first in the ring, so rotation reduces to putting the new key first, retaining every key a still-live session could have been encrypted under, and redeploying — the natural session-expiry/refresh cycle finishes the migration for you within one `OIDC_SESSION_MAX_AGE_SECONDS` window, without a separate operator step. Drop a retired key from the ring only after that same window has fully elapsed since it stopped being first — a session created just before rotation and never refreshed could otherwise become permanently unreadable (a forced re-login, not data loss, but still avoidable). Never paste these keys into tickets, logs, or the development journal either.
